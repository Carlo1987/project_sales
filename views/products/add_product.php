...
<?php if(isset($_GET['id'])) {
      ...
      $url = 'index.php?controller=Products_controller&action=add&id='.$product->id;  
      $submit = 'Modifica prodotto';
      $text = "modificare";

      if($_SESSION['identity']->id != 1 && $product->id <= 48){
        echo "<h2>...Caricamento...</h2> <script> window.location.href='https://localhost/php&sql/progetti/progetto_vendita/index.php?' </script>";
      }

} else {
      $title = 'Aggiungi Prodotto';
      $submit = 'Aggiungi prodotto';
      $url = 'index.php?controller=Products_controller&action=add';
      $text = "aggiungere";
}  ?>
...
<form action="<?=$url?>" method="POST" enctype="multipart/form-data" class="type">
    ...
    if(isset($_SESSION['product_error'])) {  echo  $_SESSION['product_error'];  }

      Utils::delete_errors_product();    ?>
